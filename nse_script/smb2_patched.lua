...
function negotiate_v2(smb, overrides)
  ...
  if is_0311 then
    ...
    data = data .. string.pack("<I4 I2 I2",
                    ...
                    0x3,            -- NegotiateContextCount (2 bytes)
  ...
  if is_0311 then
    ...
    local negotiate_context_list, context_data_preauth, context_data_encryption, context_data_compression

    -- We now add SMB2_PREAUTH_INTEGRITY_CAPABILITIES
    -- We add the padding between contexts so they are 8 byte aligned
    context_data_preauth = string.pack("<I2 I2 I2 I16 I16",
    ...
    data = data .. string.pack("<I2 I2 I4",
                    ...
                    #context_data_preauth,                                      -- DataLength (2 bytes)
                    0x0                                                -- Reserved (4 bytes)
    ) .. context_data_preauth
    ...
    -- We set SMB2_ENCRYPTION_CAPABILITIES
    -- We add the padding between contexts so they are 8 byte aligned
    context_data_encryption = string.pack("<I2 I2 I2",
    ...
    data = data .. string.pack("<I2 I2 I4",
                    ...
                    #context_data_encryption,                   -- DataLength (2 bytes)
                    0x0                                         -- Reserved (4 bytes)
    ...
    -- We set SMB2_COMPRESSION_CAPABILITIES
    -- We add the padding between contexts so they are 8 byte aligned
    context_data_compression = string.pack("<I2 I4 I2 I2",
                    0x1,        -- CompressionCount (2 bytes): 1 compression type is checked
                    0x00000000, -- Reserverd (4 bytes)
                    0x0000,     -- Reserverd (2 bytes)
                    0x0001      -- Compression Type (2 bytes) (1=LZNT1, 2=LZ77, 3=LZ77+Huffman)
                  )
    data = data .. string.pack("<I2 I2 I4",
                    smb2_values['SMB2_COMPRESSION_CAPABILITIES'],-- ContextType (2 bytes)
                    #context_data_compression,                   -- DataLength (2 bytes)
                    0x0                                          -- Reserved (4 bytes)
                  ) .. context_data_compression                  -- Data (SMB2_ENCRYPTION_CAPABILITIES)
  ...
  -- Parse negotiate context
  stdnse.debug2("SMB: Negotiate context offset is at %d", neg_context_offset)

  -- Parse is negotiate-context compression capable
  --  Last bytes:
  --    - 4 bytes reserved, 2 bytes resereved, 2 bytes: compression type (0 if not supported)
  if data:sub(-8, -1) == '\x00\x00\x00\x00\x00\x00\x01\x00' then
    smb['is_support_compresson_lznt1'] = true
  else
    smb['is_support_compresson_lznt1'] = false
  end
